...
export const PortfolioSection: React.FC<PortfolioSectionProps> = ({ onOpenLightbox, onBookShoot }) => {
  ...
  return (
    <section id="portfolio" className="w-full py-20 lg:py-28 bg-surface-container-low border-y border-outline/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-16">
                ...
                <button
                  ...
                  className={`px-4 sm:px-5 py-2 font-label-caps text-xs uppercase tracking-widest transition-all duration-200 cursor-pointer border ${
                    isActive
                      ...
                      : 'bg-surface text-secondary hover:text-on-surface hover:bg-surface-container border-surface-container-highest/80 hover-underline-anim'
                  }`}
                >
                  {tab.label}
                </button>
        ...
        <motion.div layout className="grid grid-cols-1 md:grid-cols-12 gap-6 lg:gap-8">
          <AnimatePresence>
            {filteredItems.map((item: PortfolioItem, i) => {
        ...
        >
          <button
            onClick={onBookShoot}
            className="inline-flex items-center gap-3 px-8 sm:px-12 py-4 bg-on-surface text-surface hover:bg-primary-container hover:text-on-primary font-label-caps text-xs uppercase tracking-widest transition-colors duration-300 shadow-md cursor-pointer"
          >
